...
x_Mag_trip = df_imu_trip['Magnetic.x']
y_Mag_trip = df_imu_trip['Magnetic.y'] 
z_Mag_trip = df_imu_trip['Magnetic.z'] 
yaw_trip = df_imu_trip['Yaw'].to_numpy()
...
#start = 66*fs # Error time periods
#stop = 162*fs  # 2650 to 6500

#  PLOTTING CORRECTED VS RAW YAW
    # PLOTING 
plt.figure(figsize=(7, 7))
plt.xlabel("Time in seconds")
plt.ylabel("Yaw in radians")
plt.plot(time, np.unwrap(raw_yaw), label = "Raw Yaw", color = 'blue', alpha=0.75)
plt.plot(time, np.unwrap(cal_yaw), label = "Caliberated Yaw", color = 'red', alpha=0.75)
