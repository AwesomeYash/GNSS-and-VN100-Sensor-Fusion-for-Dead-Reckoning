...
x_Mag_trip = df_imu['Magnetic.x']
y_Mag_trip = df_imu['Magnetic.y'] 
z_Mag_trip = df_imu['Magnetic.z'] 
...
raw_yaw = (np.arctan2(ymag_complete, xmag_complete))
...
"""
# PLOT: CALIBERATED MAGNETOMETER YAW VS RAW YAW (MAGNETOMETE
plt.figure(figsize=(7, 7))
plt.xlabel("Time in seconds")
plt.ylabel("Yaw in radians")
plt.plot(time, raw_yaw, label = "Raw Yaw", color = 'blue', alpha=0.75)
plt.plot(time, cal_yaw, label = "Caliberated Yaw", color = 'red', alpha=0.75)
...
gyro_yaw = df_imu_trip["Angular.z"].to_numpy()
gyro_yaw_new = gyro_yaw
yaw = scipy.integrate.cumulative_trapezoid(gyro_yaw_new, time, initial=0)
...
plt.xlabel("Time in seconds")
plt.ylabel("Yaw in radians")
plt.plot(time,cal_yaw, label = "Calibrated Magnetometer Yaw", color = 'orange')
plt.plot(time,yaw, label = "Yaw Integrated from Gyro")
plt.legend()
plt.show()
