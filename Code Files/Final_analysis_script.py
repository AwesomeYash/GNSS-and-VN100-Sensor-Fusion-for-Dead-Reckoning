...
plt.xlabel('Magnetic X')
plt.ylabel('Magnetic Y')
plt.title("Corrected MagX vs MagY")
plt.grid()
plt.legend()
plt.show()
...
plt.xlabel("Time in seconds")
plt.ylabel("Yaw in radians")
plt.title("Yaw Integrated from Gyro vs Complimentary filtered yaw")
plt.grid()
plt.legend()
plt.show()

### ------------------------- HEADING ESTIMATIONS DONE -------------------------###

### ------------------------- PART 2 : FORWARD VELOCITY ESTIMATIONS -------------------------###


# COMBINING SECONDS AND NSEC INTO A SINGLE COLUMN (for both IMU and GPS)
df_imu_trip['time'] = df_imu_trip['seconds'] + df_imu_trip['nanoseconds'] / 1e9
df_gps_trip['time'] = df_gps_trip['seconds'] + df_gps_trip['nanoseconds'] / 1e9
imu_time = df_imu_trip['time'].to_numpy()
gps_time = df_gps_trip['time'].to_numpy()

# Calculate IMU-based velocity by integrating Linear.x (forward acceleration)
linear_acc_x = df_imu_trip['Linear.x'].to_numpy()

# REMOVE MEAN ACCELERATION FOR ANY STATIC OFFSET
...
# TAKING UTM COORDINATES
utm_easting = df_gps_trip['UTM Easting'].to_numpy()
utm_northing = df_gps_trip['UTM Northing'].to_numpy()

# COMPUTE DISTANCE BETWEEN POINTS AND THEN VELOCITY
...
# PLOTTING VELOCITY ESTIMATES (IMU-based and GPS-based velocities)
"""
plt.figure(figsize=(12, 6))
plt.plot(imu_time, forward_velocity_imu, label='IMU-based Velocity (RAW)', color='blue')
...
# ZERO NEGATIVE VELOCITIES (IMU-based velocity)
...
"""
plt.figure(figsize=(12, 6))
plt.plot(imu_time, adjusted_velocity_imu, label='Adjusted IMU-based Velocity (No Filter)', color='blue')
...
### ------------------------- FORWARD VELOCITY ESTIMATIONS DONE -------------------------###
...
# DISPLACEMENT FROM Adjusted IMU AND GPS 
...
# COMPARING DISPLACEMENTS OF IMU AND GPS

plt.plot(imu_time, displacement_imu, label='IMU DISPLACEMENT', color='orange')
plt.plot(gps_time, displacement_gps, label='GPS DISPLACEMENT', color='blue')
...
# VARIABLES ASSIGNES AS GUIDED
x_dotdot = linear_acc_x
x_dot = scipy.integrate.cumulative_trapezoid(x_dotdot, imu_time, initial =0)
y_dotdot = x_dot * yaw
y_obs = df_imu_trip['Linear.y']
fv = adjusted_velocity_imu
yc = complimentary_filtered

# PLOT : Y_observed v/s wX_dot
"""
plt.figure(figsize=(10,10))
plt.plot(y_obs, label='Y observed', color='blue')
plt.plot(y_dotdot/-1, label='wX_dot', color='orange')
plt.legend()
plt.title('Y_observed vs wX_dot')
...
# NEW VARIABLE TO CALCULATE CALIBRATED YAW
new_x_magc = []    
new_y_magc = []
phi += 1.571
...
# CALIBERATING YAW ACCORDING TO MAGNETIC FEILD DATA
for i in range(len(xmag_complete)):
    an2 = R@[xmag_complete[i] - centre[0], ymag_complete[i] - centre[1]]
    #an2 = R1@[an1[0]/scale, an1[1]]
...
# ROTATE TO GET EASTING AND NORTHING VELOCITY COMPONENTS
Ve = adjusted_velocity_imu * np.cos(cal_yaw)
Vn = adjusted_velocity_imu * np.sin(cal_yaw) 

# INTEGRATE TO GET EASTING AND NORTHING
Xe = scipy.integrate.cumulative_trapezoid(Ve * scale, imu_time, initial=0)
Xn = scipy.integrate.cumulative_trapezoid(Vn * scale, imu_time, initial=0)
...
"""
plt.figure(figsize=(12, 6))
plt.plot(utm_easting, utm_northing, label='GPS Trajectory', color='orange')
plt.plot(Xe, Xn, label='IMU Trajectory', color='blue')
plt.title('Trajectory Comparison: IMU vs GPS')
